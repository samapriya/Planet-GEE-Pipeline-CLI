    ...
    for filename in os.listdir(indir):
        if filename.endswith(".tif"):
            ...
            try:
                ...
                resampling_kernel = xmldoc.getElementsByTagName("ps:resamplingKernel")[
                    0
                ].firstChild.data
                ...
                psilaz = xmldoc.getElementsByTagName("opt:illuminationAzimuthAngle")[
                    0
                ].firstChild.data
                psilelv = xmldoc.getElementsByTagName("opt:illuminationElevationAngle")[
                    0
                ].firstChild.data
...
def srmeta_from_parser(args):
    srmeta(
        indir=args.indir, mfile=args.mfile, errorlog=args.errorlog, folder=args.folder
    )


def main(args=None):
    parser = argparse.ArgumentParser(
        "Tool to process PlanetScope Surface Reflectance Data"
    )
