...
def aoijson(start, end, cloud, inputfile, geo, loc):
    if inputfile == "KML":
        ...
        print(
            "New structured JSON has been created at "
            + str(
                os.path.join(
                    os.path.join(
                        os.path.splitext(loc)[0],
                        os.path.splitext(os.path.basename(geo))[0] + "_aoi.json",
                    )
                )
            )
        )
        ...
        with open(geo) as insert, open(os.path.join(dir_path, "aoi.json")) as jbase:
            ...
            if (len(str(cinsert).split(",")) / 2) <= 10:
                geombase["config"][0]["config"][
                    "coordinates"
                ] = cinsert  # coordinate insert
                geombase["config"][2]["config"]["gte"] = (
                    str(start) + "T04:00:00.000Z"
                )  # change start date
                geombase["config"][2]["config"]["lte"] = (
                    str(end) + "T03:59:59.999Z"
                )  # change end date
                geombase["config"][3]["config"][
                    "gte"
                ] = 0  # change cloud minima default=0
                geombase["config"][3]["config"]["lte"] = float(
                    cloud
                )  # change cloud maxima
                print(
                    os.path.join(
                ...
                print(
                    ...
                    + str(
                        os.path.join(
                            os.path.join(
                                os.path.splitext(loc)[0],
                                os.path.splitext(os.path.basename(geo))[0]
                                + "_aoi.json",
                            )
                        )
                    )
                ...
                cbox = []
                cbox.append(
                    ee.Geometry.Polygon(cinsert).bounds().getInfo()["coordinates"][0]
                )
                geombase["config"][0]["config"][
                    "coordinates"
                ] = cbox  # coordinate insert
                geombase["config"][2]["config"]["gte"] = (
                    str(start) + "T04:00:00.000Z"
                )  # change start date
                geombase["config"][2]["config"]["lte"] = (
        ...
        for line in raw:
            ...
            filenames = (
                p1 + f2 + p2 + str(start) + p3 + str(end) + p4 + p5 + str(cloud) + p6
            )
        with open(
            os.path.join(os.path.split(geo), os.path.splitext(loc)[0] + "_aoi.json"),
            "w",
        ) as outfile:
        ...
        for line in raw:
            ...
            l4 = str(l3).replace(" ", ",")
            filenames = (
                p1 + l4 + p2 + str(start) + p3 + str(end) + p4 + p5 + str(cloud) + p6
            )
        with open(
            os.path.join(os.path.split(geo), os.path.splitext(loc)[0] + "_aoi.json"),
            "w",
